...
$$\tan(2\theta) = \frac{2a^{(k-1)}_{pq}}{a^{(k-1)}_{pp} - a^{(k-1)}_{qq}}.$$ \cite{NLA}

From this we can retrieve the angle and obtain the rotation matrix in each iteration.
...
Figure \ref{j-algo} visualizes the progress of the \textit{Jacobi}-method on a symmetric $5 \times 5$ matrix. As we can see the matrix converges to a diagonal matrix already after 10 iterations.
